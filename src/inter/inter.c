...
static int const INTER_STEPS[] = { // Semitone distance between major intervals
    0, 2, 4, 5, 7, 9, 11, 12
};
...
mah_get_inter(struct mah_note const note, struct mah_interval const interval, enum mah_error* err)
{
    ...
    int inter   = (interval.steps - 1) % ADJUST_NOTE,
        pitch   = note.pitch + (interval.steps / SIMPLE_INTER_MAX),
        root    = note.tone,
        quality = interval.qual;

    int semi = 0;
    if (inter == 0 || inter == 3 || inter == 4 || inter == 7) { // if perfect interval
        switch (quality) { // special semitone adjustment for perfect interval
            case MAH_AUGMENTED:
                semi = 0;
                break;
            case MAH_DIMINISHED:
...
mah_return_inter(struct mah_note const note_a, struct mah_note const note_b, enum mah_error* err)
{
    ...
    int quality = (to_semitone(note_b.tone) + note_b.acci + pitch * SIZE_CHROMATIC) -
                  (to_semitone(note_a.tone) + note_a.acci + note_a.pitch * SIZE_CHROMATIC) -
                  INTER_STEPS[steps];

    if (quality >= 11 && quality <= 13) { // treated as 1 so must adjust (C0 12 = C1 0)
        quality -= SIZE_CHROMATIC;
    } else if (quality < -2 || quality > 1) { // between diminished and augmented
