...
{
    for (int n = 0; n < num; n++)
    { // instead of shifting scale up to match, shift semitones down
...
get_enharmonic(struct mah_note note) // only works for sharps and flats
{
    ...
    return (struct mah_note) {
        .tone = enharmonic,
        .acci = -acci *
                ((acci == MAH_SHARP && tone != MAH_B && tone != MAH_E) ||
                 (acci == MAH_FLAT && tone != MAH_C && tone != MAH_F)), // cases for notes with no black keys between
    };
}